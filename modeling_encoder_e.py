...
VISION_ENCODER_PATH=os.path.join(ENCODER_PATH, "vision")
...
class VisionT5MeanBiEncoderForClassification(BiEncoderBase):
    _ENCODER_TYPE = 'biencoder'

    def __init__(self, args=None, vision_encoder=None, language_encoder=None):
        super(VisionT5MeanBiEncoderForClassification, self).__init__(
            args=args,
            vision_encoder=vision_encoder,
            language_encoder=language_encoder
        )
        ...
        self.classification_head = nn.Linear(
            self.vision_encoder.config.hidden_size + self.language_encoder.config.hidden_size,
            args.num_classes
        )

    def load_weight_from_args(self, args):
        self.vision_encoder = ViTModel.from_pretrained(VISION_ENCODER_PATH)
        self.language_encoder = T5EncoderMean.from_pretrained(LANGUAGE_ENCODER_PATH)
    ...
    def forward(self, batch):
        ...
        concat_repr = torch.cat([language_repr, vision_repr], dim=-1)

        logits = self.classification_head(concat_repr)

        return {'logits': logits}

class VisionT5MeanBiEncoderForClassification2(BiEncoderBase):
    _ENCODER_TYPE = 'biencoder'

    def __init__(self, args=None, vision_encoder=None, language_encoder=None):
        super(VisionT5MeanBiEncoderForClassification2, self).__init__(
            args=args,
            vision_encoder=vision_encoder,
            language_encoder=language_encoder
        )
